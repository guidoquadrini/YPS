<?php
$this->set_css($this->default_theme_path . '/q-grid/css/bootstrap.css');
...
$this->set_js($this->default_theme_path . '/q-grid/js/cookies.js');
$this->set_js($this->default_theme_path . '/q-grid/js/bootstrap.js');

$this->set_js($this->default_javascript_path . '/jquery_plugins/jquery.form.min.js');

$this->set_js($this->default_javascript_path . '/jquery_plugins/jquery.numeric.min.js');
$this->set_js($this->default_theme_path . '/q-grid/js/jquery.printElement.min.js');
...
<div id='list-report-success' class='report-div success report-list' <?php if ($success_message !== null) { ?>style="display:block"<?php } ?>><?php if ($success_message !== null) { ?>
    ...
    <?php }
    ?></div>
<div class="panel panel-default" style='width: 95%;margin: 0 auto;' data-unique-hash="<?php echo $unique_hash; ?>">
    ...
    <div id='main-table-box' class="main-table-box">
            ...
            <div>
                <?php if (!$unset_add) { ?>
                    <a href='<?php echo $add_url ?>' title='<?php echo $this->l('list_add'); ?> <?php echo $subject ?>' class='add-anchor add_button'>
                        <button class="btn btn-primary"><span class="glyphicon glyphicon-plus"> <?php echo $this->l('list_add'); ?> <?php echo $subject ?></span></button>
                    </a>
                <?php } ?>
                <?php if (!$unset_export) { ?>
                    <a class="export-anchor" data-url="<?php echo $export_url; ?>" target="_blank">
                        <button class="btn btn-success"> <span class="glyphicon glyphicon-floppy-save"> <?php echo $this->l('list_export'); ?></span></button>
                    </a>                        
                <?php } ?>
                <?php if (!$unset_print) { ?>
                    <a class="print-anchor" data-url="<?php echo $print_url; ?>">
                        <button class="btn btn-success"> <span class="glyphicon glyphicon-print"> <?php echo $this->l('list_print'); ?></span></button>
                    </a>
                <?php } ?>
            </div>
        ...
        <div id='ajax_list' class="ajax_list"><?php echo $list_view ?></div>
            ...
            <?php //Inicia el footer.
        echo form_open($ajax_list_url, 'method="post" id="filtering_form" class="form-inline filtering_form" autocomplete = "off" data-ajax-list-info-url="' . $ajax_list_info_url . '"');
        ?>
        <div class="form-group quickSearchBox" id='quickSearchBox'>

<?php echo $this->l('list_search'); ?>: <input type="text" class="form-control qsbsearch_fieldox search_text" name="search_text" size="30" id='search_text'>
...
<?php } ?>
            ...
            <span class='search-div-clear-button'>
                <input type="button" value="<?php echo $this->l('list_clear_filtering'); ?>" id='search_clear' class="btn btn-danger search_clear">
            </span>
        </div>
...
<?php echo $entries_lang_string; ?>
                ...
                <div>
                    <span class="pcontrol"><?php echo $this->l('list_page'); ?> <input name='page' type="text" value="1" size="4" id='crud_page' class="form-control crud_page">
